...
export default function ActivityCard({ activity, onComplete, completing }) {
  ...
  const duration =
    ...
    activity?.minimum_duration_minutes ||
  ...
  return (
    <article className="activity-card">
      <div className="activity-card-header">
        <div>
          <span className={`tag tag-${activity?.type || "custom"}`}>
            {activity?.type || "custom"}
          </span>
          <h3>{activity?.name || activity?.title || "Atividade sem nome"}</h3>
        </div>

        <strong className="xp-badge">+{activity?.xp || 0} XP</strong>
      </div>
